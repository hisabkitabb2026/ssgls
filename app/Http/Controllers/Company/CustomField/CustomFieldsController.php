...
class CustomFieldsController extends Controller
{
    ...
    /**
     * Auto-create transport invoice custom fields if they don't exist yet.
     */
    private function ensureTransportInvoiceFields(Request $request): void
    {
        ...
        $fieldNamesByModel = collect($fields)->map(fn ($definitions) => collect($definitions)->pluck('name')->all())->all();
    ...
    /**
     * Get all transport invoice slugs (optionally filtered by template name).
     *
    ...
    /**
     * Get the field definitions for a given transport invoice template.
     *
        ...
        if ($templateName === 'lr_receipt') {
            return [
                'Invoice' => [
                    ...
                    // Consignment details — all at Invoice level since LR Receipts
                    // have no line items
        ...
        if ($templateName === 'lorry_receipt') {
            return [
                'Invoice' => [
                    ...
                    ['name' => 'Received No Of Bilties', 'type' => 'Input', 'default_answer' => 'LR NO 1937*471 wt Orthopedic Goods'],
                ],
                'Item' => [],
            ];
        }
