...
function openProfileCreate(close?: () => void): void {
  close?.()
...
function handleProfileSaved({ profile }: { type: string; profile: LorryPartyProfile }): void {
  selectedProfile.value = profile
  invoiceStore.newInvoice.party_profile_id = profile.id ?? null

  // Auto-fill "Paid To" and "Final Paid To" with the newly created party's name
  invoiceStore.newInvoice.paid_to = profile.name ?? null
  invoiceStore.newInvoice.final_paid_to = profile.name ?? null
}
